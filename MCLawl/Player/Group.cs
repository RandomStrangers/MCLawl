...
namespace MCLawl
{
    public class Group
    {
        ...
        public static void InitAll()
        {
                                ...
                                else
                                {
                                    switch (property.ToLower())
                                    {
                                        case "rankname":
                                            gots = 0;
                                            thisGroup = new Group();
                                             if (GroupList.Find(grp => grp.name == value.ToLower()) == null)
                                        ...
                                        case "permission":
                                            ...
                                            if (foundPermission > 150 || foundPermission < -50)
                                            {
                                                Server.s.Log("Permission must be between -50 and 150 for ranks");
                                                break;
                                            }
            ...
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Owner) == null) GroupList.Add(new Group(LevelPermission.Owner, int.MaxValue, "Owner", '4', "120_rank.txt"));
            GroupList.Add(new Group(LevelPermission.Nobody, int.MaxValue, "Nobody", '0', "nobody.txt"));
        ...
        public static string concatList(bool includeColor = true, bool skipExtra = false, bool permissions = false)
        {
            ...
            foreach (Group grp in Group.GroupList)
            {
                if (!skipExtra || (grp.Permission > LevelPermission.Guest && grp.Permission < LevelPermission.Owner))
                    if (includeColor) {

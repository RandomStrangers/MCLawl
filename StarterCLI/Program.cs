    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (tries > 4)
            {
                ...
                MessageBox.Show("Unable to download MCLawl_.dll.  Please download it manually at " + DLLLocation + ", place it in the same folder as this executable, and restart this application", "Required DLL Missing", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Console.ReadLine();
                Console.WriteLine("Bye!");
            }
            else if (File.Exists("MCLawl_.dll"))
            {
                string[] args1 = new string[] { "cli" };
                //Crash issue fixed by re-executing the exe to properly load MCLawl_.dll.
                if (!needsToRestart)
                    openServer(args1);
                else
            ...
            else
            {
                ...
                try
                {
                    ...
                    Client.DownloadFile(DLLLocation, "MCLawl_.dll");
                    Client.Dispose();
                ...
                {
                    tries = 5;
                    Console.WriteLine("\nAn error occured while attempting to download MCLawl_.dll\n");
                    Main(args);
                }
            }
        }
